...
def transform_data(gamestate):
    ...
    def f(x): return player_info[x]['matrix']
...
def player_handle_outcome(game_info):
    ...
    save_files = [int(x.split('_')[-1]) for x in listdir(join(dirname(__file__),
                                                              'data_store', 'state_data')) if x.startswith(prefix)]
    # print(game_info)
    ...
    with open(join(dirname(__file__), 'data_store', 'state_data', file_name)) as f:
        ...
        dic['player1_hand'] = ' | '.join(
            [x.card_string() for x in game_info['player_cards'][0]['player 1']['cards']])
        dic['player2_hand'] = ' | '.join(
            [x.card_string() for x in game_info['player_cards'][1]['player 2']['cards']])
...
def model_reader(gamestate, fn_name):
    ...
    pp = PokerPlayer(model_paths, gamestate['player_info']['state'], transform_data(
        gamestate), flatten=flatten)
...
def model_user(gamestate, flatten=True):
    ...
    model_number = max([int(x.split('_')[-1])
                        for x in listdir(join(dirname(__file__), 'trained_models')) if f(x)])
    if flatten:
        addon = '-flatten'
    else:
        addon = ''
    ...
    model_paths = {
        x: f'{x}{addon}_{model_number}' for x in 'flop turn river preflop'.split()}
    pp = PokerPlayer(model_paths, gamestate['player_info']['state'], transform_data(
        gamestate), flatten=flatten)
    perc = pp.apply_model(transform_data(gamestate))
    perce = perc.detach().numpy()[0]
...
def run_model(gamestate, prefix):
    flatten = 'flatten' in prefix
    model_paths = {
        x: f'{x}{prefix}' for x in 'flop turn river preflop'.split()}
    pp = PokerPlayer(model_paths, gamestate['player_info']['state'], transform_data(
        gamestate), flatten=flatten)
    perc = pp.apply_model(transform_data(gamestate))
    perce = perc.detach().numpy()[0]
...
def playerchoice(gamestate):
    ...
    with open(join(dirname(__file__), 'data_store', 'state_data', file_name)) as f:
        dic = json.load(f)
        dic[gamestate['player_info']['state']] = float(
            perc.detach().numpy()[0])
    with open(join(dirname(__file__), 'data_store', 'state_data', file_name), 'w') as f:
        f.write(json.dumps(dic))
